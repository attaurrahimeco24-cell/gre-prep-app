...
def render_exam_simulation(test_id: str) -> None:
    ...
    # If no active sections remain, the test is over.
    if not sec_info:
        st.balloons()
    ...
    # Initialize section state if it is a new section
    ...
    # Handle empty fallback edge-case gracefully
    if not questions:
    ...
    if time_status["is_expired"]:
        ...
        testing_engine.complete_section_and_adapt(test_id, sec_instance_id)
        st.session_state.pop("current_section_payload", None)
        time.sleep(1.5) # Brief pause so user reads the warning before the page changes
        st.rerun()
    ...
    # ---------------------------------------------------------
    # AUTOSAVE LOGIC: Prevents selections from clearing themselves
    # ---------------------------------------------------------
    ...
    def autosave_answer():
        ...
        if val is not None:
            st.session_state["user_answers"][q_id] = val # Save to local session
            time_spent = timer.get_question_elapsed_time(st.session_state.get("q_start_time", time.time()))
            testing_engine.submit_answer_atomically(test_id, sec_instance_id, q_id, str(val), time_spent) # Save to SQLite
            st.session_state["q_start_time"] = time.time()

    # Render inputs tied directly to the autosave callback
    if q_type == "Numeric Entry":
        ...
        st.radio(
            ...
            index=idx_val, 
            key=widget_key, 
            on_change=autosave_answer
        )

    st.divider()
    
    # Navigation Footer (Next no longer handles saving, it only navigates)
    ...
    if c3.button("💾 Save", use_container_width=True):
        # Explicit save button for user reassurance (though autosave already handled it)

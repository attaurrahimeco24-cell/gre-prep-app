...
def live_timer_fragment(section_name: str, sec_instance_id: str, duration_seconds: int, test_id: str):
    ...
    if time_status["is_expired"]:
        st.error("⏱️ Time expired for this section! Auto-submitting...")
...
def render_exam_simulation(test_id: str) -> None:
    ...
    if not questions:
        st.error("No questions available. Please skip to the next section.")
        if st.button("⏭️ Skip Section", type="primary"):
            testing_engine.complete_section_and_adapt(test_id, sec_instance_id)
            st.session_state.pop("current_section_payload", None)
            st.rerun()
        return
    ...
    with col_q1:
        ...
        st.caption(f"Domain: **{current_q['domain']}**")
    with col_q2:
        mark_key = f"mark_{q_id}"
        def toggle_mark():
            st.session_state["marked_for_review"][q_id] = st.session_state[mark_key]
        st.checkbox("🚩 Mark for Review", value=st.session_state.get("marked_for_review", {}).get(q_id, False), key=mark_key, on_change=toggle_mark)

    # Premium Typography for Question Text
    st.markdown(
        f"""
        <div style='font-size: 1.2rem; font-weight: 500; line-height: 1.6; padding: 20px; background-color: var(--secondary-background-color); border-radius: 8px; border: 1px solid var(--border-color, #e2e8f0); margin-bottom: 24px;'>
    ...
    if q_type == "Numeric Entry":
        ...
        st.text_area("Write your essay response here (saves automatically):", value=existing_ans, height=350, key=widget_key, on_change=autosave_answer)
    else:
    ...
    if c3.button("💾 Save", use_container_width=True):
        if widget_key in st.session_state: autosave_answer()
